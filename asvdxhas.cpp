...
//Made by GitHub users @sarthak1905 and @vishruth-v
...
public:
	...
	void changekey(char* newkey) { strcpy(key, newkey); }
	message(char* from, char* to, char* attachment, char* keyb)
	{
		strcpy(sender, from);
		strcpy(receiver, to);
		strcpy(attachment_name, attachment);
		strcpy(key, keyb);
	}
	message() {}
};
...
{
	strcpy(received_file_name, username);
	strcat(received_file_name, "_received.dat");
	int cnt = 0;
	message m1;
	fstream fread;
...
void user::openfile()
{
	...
	cout << "\nEnter message no. you wish to open"; 
    cin >> no;
	fstream fread;
    cout << "sjscnjs\n";
	fread.open(received_file_name, ios::in | ios::binary);
	...
	cout << m1.getattachment();
    cout << "sjscnjs\n";
	strcpy(attachment, m1.getattachment());
	strcpy(key, m1.getkey());

	fstream readattach;
	readattach.open(attachment, ios::in | ios::binary);
	strcpy(decrypted_filename, attachment);
	strcat(decrypted_filename, "_decrypted.txt");
...
char* user::generate_key_send(message ob)
{
	char plainkey[40];
	char cipherkey[40];
	strcpy(plainkey, ob.getreceiver());
	strcat(plainkey, username); //first receiver, then sender
	strcpy(cipherkey, plainkey);
